	...
	<Heading tag="h2" size="lg">
		...
		{#each slice.primary.item as item}
			<div class="ml-6 mt-8 max-w-prose md:ml-12 md:mt-16">
				...
				<p class="mt-1 flex w-fit items-center gap-1 text-2xl font-semibold tracking-tighter text-slate-400">
					<span>{item.time_period}</span>{' '}
					<span>/</span>{' '}
					<span class="text-3xl font-extralight">{item.institution}</span>
				</p>
